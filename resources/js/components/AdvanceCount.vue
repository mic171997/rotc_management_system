...
export default {
  ...
  watch: {
    date(value) {
      const hasNull = value.some(value => value === null)
  ...
  methods: {
    ...
    async generateBtnEXCEL(e, reportType) {
      ...
      if (reportType == 'CountData') {
        report = '&report=Excel'
        pass = '/reports/advance_count/generateAppDataExcel'
      ...
      const { headers, data } = await axios.get(
        pass +
          `?date=${btoa(this.date)}&date2=${btoa(this.date2)}&vendors=${btoa(
            ...
            this.department
          }&section=${this.section}&countType=${this.countType}&type=ADVANCE` +
          report,
        {
          responseType: 'blob'
        }
    ...
    async generateBtn(e) {
      ...
      const countdata = this.data.countData
      const thisButton = e.target
      const oldHTML = thisButton.innerHTML
      ...
      const { headers, data } = await axios.post(
        `/reports/advance_count/generate?`,
        {
      ...
      link.setAttribute(
        'download',
        `Advance Count (APP) as of ${this.date} ${this.business_unit} ${this.department} ${section}.pdf`
      )
      // console.log(link)
      document.body.appendChild(link)
      link.click()
    ...
    async getDates() {
      ...
      const response = await axios.get(
        `/reports/appdata/getDates?comp=${this.company}&bu=${this.business_unit}&dept=${this.department}&sect=${this.section}&countType=ADVANCE&type=${this.countType}`
      )
