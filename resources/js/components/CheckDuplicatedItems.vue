									...
									<div class="row pad-all" style="padding-left: 10px">
										...
										<div class="col-lg-3">
											...
											<date-picker v-model="date" type="date" :disabled-date="
                                                ...
                                                " format="YYYY-MM-DD" value-type="YYYY-MM-DD" :placeholder=" loadingDate ? 'RETRIEVING DATE PLS WAIT...' : !availableDates.length ? 'No dates available.' : 'Select date range' " 
                                                :disabled=" !company || !business_unit || !department || !section || !countType || loadingDate || !availableDates.length
                                            "></date-picker>
										</div>
									</div>
						...
						<table class="table table-striped table-vcenter" id="data-table">
							...
							<tbody>
								...
								<tr v-for="(datas, index) in getdata.data" :key="index">
									...
									<td>
										<button class=" btn btn-info btn-xs icon-lg add-tooltip" data-original-title="Edit"
											data-container="body" data-toggle="modal" data-target="#exampleModal">
											<i class="fa fa-pencil-square-o"
												@click="editBtn(datas.id, buttonclick = 'edit')"></i></button>
									</td>
								</tr>
							</tbody>
						</table>
				...
				<div class="modal-dialog modal-lg" role="document">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title" id="exampleModalLabel">Check Duplicated Items</h5>
						...
						<div class="modal-body">
							...
							<div class="flex flex-wrap -mx-3 mb-3" v-show="buttonclick == 'edit'">
								<div class="w-full md:w-1/2 px-3 mb-3 md:mb-0">
									<label>Number of Duplicated Items:</label>
									<label class="bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">2</label>
								</div>
								<div class="w-full md:w-1/2 px-3 mb-3 md:mb-0">
									<label>Actions:</label>
									<br>
									<button type="submit" class="btn btn-info" @click="deleteBtn()">
										Clear	
									</button>
							...
							<div class="flex flex-wrap -mx-3 mb-3">
								...
								<div class="w-full md:w-1/2 px-3 mb-3 md:mb-0" v-show="buttonclick == 'delete'">
									<label>Reason for Adjustment:</label>
									<textarea id="message" rows="4" v-model="reason"
										class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
										placeholder="Write your reasons here..."></textarea>
								</div>
								<div class="w-full md:w-1/2 px-3 mb-3 md:mb-0">

									<div v-show="buttonclick == 'delete'">
										<label>Actions:</label>
										<br>
										<button type="submit" class="btn btn-danger" :disabled="!reason"
											@click="deleteBtn()">
											Delete
										</button>
									</div>
...
export default {
	...
	methods: {
		...
		editBtn(id) {
			// axios.get(`/reports/backendadjustment/getinfo?id=${id}&bu=${this.business_unit}&dept=${this.department
			// 	}&section=${this.section}&type=${this.Type}`).then(response => {
		...
		deleteBtn() {
			...
			swalWithBootstrapButtons.fire({
				...
				confirmButtonText: this.buttonclick == 'edit' ? 'Yes, update it!' : 'Yes, delete it!',
				cancelButtonText: 'No, cancel!',
				reverseButtons: true
			}).then((result) => {
				if (result.isConfirmed) {
					axios.post(`/reports/backendadjustment/getdeleteinfo?data=${this.getinfo[0].id}&c_qty=${this.getinfo[0].conversion_qty}&qty=${this.getinfo[0].qty}&newqty=${this.newqty}&newuom=${this.newuom}&countType=${this.Type}&bu=${this.business_unit}&dept=${this.department}&sect=${this.section}&reason=${this.reason}&button=${this.buttonclick}&status=DELETED`).then(response => {
						swalWithBootstrapButtons.fire(
							this.buttonclick == 'edit' ? 'UPDATED!' : 'DELETED!',
							this.buttonclick == 'edit' ? 'Your file has been updated.' : 'Your file has been deleted.',
							'success'
						)
		...
		async searchresult(page = 1) {
			...
			this.isloading = true;
			await axios.get(
				`/reports/checkduplicateditems/getResults/?date=${btoa(this.date)}&type=${this.Type}&bu=${this.business_unit}&dept=${this.department
				}&section=${this.section}&countType=${this.countType}&page=` + page
			).then(response => {
		...
		async getDates() {
			...
			const response = await axios.get(
				`/reports/checkduplicateditems/getDates?comp=${this.company}&bu=${this.business_unit}&dept=${this.department}&sect=${this.section}&countType=${this.Type}`
			)
