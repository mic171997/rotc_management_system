...
class CopyPcountAppCountData implements FromView, ShouldAutoSize, WithEvents, WithColumnFormatting
{
    ...
    public function view(): View
    {
        ...
        // dd($da);
        $i = 0;
        $da['data'] = collect($da['data'])->map(function ($trans)  use ($table, $var, &$i) {

            $res = array_map(function ($x) use ($table, $var, &$i) {
                return array_map(function ($y) use ($table, $var, &$i) {
                    return array_map(function ($z)  use ($table, $var, &$i) {
                        return array_map(function ($w) use ($table, $var, &$i) {
                            $newArr = [];
                            $test = 'what';
                            $test2 = 'audit';
                            foreach ($w as $index => $xyz) {
